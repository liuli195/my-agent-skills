...
- **风险匹配验证**：验证强度必须与变更风险和影响范围匹配。功能、缺陷和集成行为变更至少从真实用户入口或发布形态运行覆盖本次可观察结果的快速冒烟；外部客户端或系统适配完成后立即运行该端真实冒烟。只有规格、风险、仓库配置或用户明确要求时，才运行完整端到端回归。文档、格式及无运行行为变化的配置改动只运行对应检查。不得用内部单元测试替代要求的用户入口冒烟。
