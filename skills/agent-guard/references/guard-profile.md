...
- 编写状态、状态权限或完成条件：读“State（状态）字段”。
- 编写 Artifact（产物）：读“Artifact（产物）字段”。
...
```powershell
python skills\agent-guard\scripts\validate_guard_profile.py <guard-profile-dir>
```
...
- `permissions` 控制当前状态允许哪些工具调用；`transitions` 控制状态完成后如何判断下一状态。运行行为见 `runtime-contract.md`。
- `state_completed` 完成证据只能来自画像声明的 artifact（产物）、状态目录、审计记录或约定路径，不得依赖 `payload.*`。
- 每个非终止状态完成后必须唯一匹配一条转换；无匹配或多匹配都是画像配置错误。
- `permissions.rules` 可以接近 Codex（Codex 代理）命令规则或 Claude（Claude 代理）权限清单，但画像不生成工具原生静态权限配置。
