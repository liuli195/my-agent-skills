...
补充验证：覆盖 validate（校验）不调用 GitHub API（GitHub 接口）/`gh` CLI（GitHub 命令行工具），配置草案不新增字段，MySpec validation（规格校验），以及 pr-flow-init（拉取请求流程初始化）到 validate（校验）和 init（初始化）写入的端到端回归。

## Spec Patch

已写入 delta spec（增量规格）：`myspec/changes/prefer-codeql-default-setup/specs/pr-flow-plugin/spec.md`。
