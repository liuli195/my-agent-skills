...
当前仓库同时维护 Agent Guard Plugin（代理守卫插件）源码、MySpec 规格、Codex marketplace catalog（市场目录）和 Claude marketplace catalog。`main` 与 `marketplace` 分支存在发布态差异，核心问题是：这些差异必须可解释、可复现、可验证，不能长期靠手工维护发布分支。
