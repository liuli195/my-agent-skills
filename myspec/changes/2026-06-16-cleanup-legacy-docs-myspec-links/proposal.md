...
`docs/adr`、`docs/changes`、`docs/designs`、`docs/plans`、`docs/prd` 中的 agent-guard 历史文档已经迁移到 MySpec baseline（基线）。这些目录继续存在，会让旧 PRD（产品需求文档）、旧 ADR（架构决策记录）、旧 design（设计）和旧 issue/change 记录继续进入活跃上下文。
...
- 直接删除已被 MySpec baseline 完整吸收、且无必要保留的旧 change/issue 文档和空索引文档。
- 将不能直接删除的历史证据移动到 `myspec/changes/archive/2026-06-16-baseline-agent-guard-docs-migration/source-docs/`，移动后原目录仍必须删除。
- 更新活跃上下文引用，让 `AGENTS.md`、`docs/agents/domain.md`、相关测试和引用文档不再依赖被删除目录。
- 明确 `myspec/specs/**` 是当前契约权威，archive 中的旧文档只用于历史追溯。

## 能力

**新增能力：**
- `legacy-docs-myspec-navigation`: 管理 MySpec baseline 迁移后旧 docs 目录的删除、必要归档、活跃引用切换和权威边界。
...
- 计划移动到 `myspec/changes/archive/2026-06-16-baseline-agent-guard-docs-migration/source-docs/` 后再删除原目录：
