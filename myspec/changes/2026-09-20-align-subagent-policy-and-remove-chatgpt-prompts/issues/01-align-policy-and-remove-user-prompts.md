...
**What to build（构建内容）：** 对齐 `subagent-policy`（子代理策略）技能、四个居民代理配置和正式规格：架构师优先使用网页 ChatGPT，失效时回退到本地具名架构师；所有角色优先使用具名代理，必要时统一回退到通用代理；模型不可用时使用宿主默认配置并声明；实际权限无法满足角色边界时停止。同步删除 Codex 用户级 `AGENTS.md` 与 Claude Code 用户级 `CLAUDE.md` 中不再需要的网页 ChatGPT 协作提示词，同时保留语言规则、网页技能和其他配置。
