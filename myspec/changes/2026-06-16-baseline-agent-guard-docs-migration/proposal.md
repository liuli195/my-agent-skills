...
现有 `docs/adr`、`docs/changes`、`docs/designs`、`docs/plans` 和 `docs/prd` 混合保存当前契约、历史 issue 说明、技术设计和草案内容。仓库已经初始化 MySpec，需要把仍有效的 Agent Guard（代理守卫）事实整理为可校验的 MySpec baseline（基线），避免后续实现继续依赖散落文档和已被 ADR（架构决策记录）替代的旧契约。

## What Changes（变更内容）

- 建立 `agent-guard-core`、`agent-guard-plugin-runtime`、`agent-guard-brief-injection` 和 `agent-guard-skill-entrypoints` 四个 MySpec capability（能力）。
- 把当前有效契约写入 `myspec/specs/<capability>/spec.md`。
- 把本次迁移过程保留为 `baseline-agent-guard-docs-migration` change（变更）并归档。
- 补齐 `myspec/config.yaml`，使用默认 `spec-driven` schema（流程模式）。
- 不创建 `myspec/project.md`，因为当前 MySpec CLI（命令行工具）把它视为 legacy（旧结构）。
...
- 无；当前仓库没有既有 MySpec main specs（主规格）。

## Impact（影响范围）

- Affected docs（受影响文档）：`myspec/config.yaml`、`myspec/changes/baseline-agent-guard-docs-migration/**`、`myspec/specs/**`。
