...
仓库内旧文档已经覆盖 Agent Guard（代理守卫）的需求、设计、ADR（架构决策记录）和 issue（议题）变更说明，但这些文档不是 MySpec 可校验格式。`myspec list --specs --json` 在迁移前没有 main spec（主规格），`myspec validate --all --strict --json` 也只校验到 0 个对象。

本次迁移的目标是建立 MySpec baseline（基线），不是重写实现。旧文档中有部分内容已被 ADR 0002 替代，例如 Subject Resolver（主体解析器）、`subject_key_hash` 身份路径、Git Hook（Git 钩子）第一版兜底、项目级复制 Runtime code（运行时代码）。这些旧契约只作为来源背景，不进入当前 baseline specs（基线规格）。

## Goals / Non-Goals（目标 / 非目标）

**Goals（目标）：**

- 建立 4 个可由 MySpec strict validation（严格校验）通过的 main specs（主规格）。
- 保留本次迁移的 proposal（提案）、design（设计）、tasks（任务）和 delta specs（增量规格）。
- 明确旧文档到 MySpec capability（能力）的映射。
...
- 不创建 `myspec/project.md`。
- 不把旧 issue（议题）重新恢复为 active change（活动变更）。

## Decisions（决策）

- MySpec 目录使用当前 CLI（命令行工具）认可的结构：`myspec/config.yaml`、`myspec/specs/<capability>/spec.md`、`myspec/changes/<change>/...` 和 `myspec/changes/archive/YYYY-MM-DD-<change>/...`。
...
- MySpec 需求文本过长会降低可读性。缓解：按 capability（能力）拆分，requirements（需求）保持行为级表达，细节留在 scenario（场景）和旧文档来源。
- 旧 docs（文档）暂时保留可能让未来读者混淆权威来源。缓解：本次先建立 MySpec 权威基线，后续单独 cleanup change（清理变更）处理旧 docs 跳转或归档。

## Migration Plan（迁移计划）

1. 创建 `baseline-agent-guard-docs-migration` active change（活动变更）。
2. 写入 proposal（提案）、design（设计）、tasks（任务）和 4 个 delta specs（增量规格）。
3. 运行 `myspec validate baseline-agent-guard-docs-migration --type change --strict --json`。
4. 按 `myspec-sync-specs` 规则把 delta specs 同步为 main specs（主规格）。
5. 运行 `myspec validate --specs --strict --json` 和 `myspec validate --all --strict --json`。
6. 将任务标记为完成。
7. 使用 `myspec archive baseline-agent-guard-docs-migration --skip-specs -y` 归档，避免重复应用已经同步的 main specs。
