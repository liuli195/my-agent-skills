...
- 最后运行 focused PR Flow（拉取请求流程）测试和 MySpec（自有规格）严格校验。

## Spec Patch

本变更已有 MySpec delta（开放规格增量）：新增 GitHub（代码托管平台）可执行远端待办要求，并修改 PR Flow init（拉取请求流程初始化）的渐进式模板契约。实现阶段不需要额外回写新的 Spec Patch（规格补丁），除非发现验收场景仍缺失。
