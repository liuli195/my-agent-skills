...
- 保留 local build contract（本地构建契约）、PR Flow（拉取请求流程）、Release Flow（发布流程）、Agent Guard（代理守卫）、cross-agent-review（跨代理审查）和 Test Framework（测试框架）关键行为覆盖，并保留 `verify.myspec`（开放规格验证）覆盖和计时，不用删测试换速度。
...
- 不创建或修改 `docs/rules/` 下任何文件；测试写法规则先沉淀在 MySpec（规格流程）产物里。
