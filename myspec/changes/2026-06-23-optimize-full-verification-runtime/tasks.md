...
- [x] 1.2 Record verify-check and pytest grouped timings for local build contract, PR Flow, Release Flow, Agent Guard, cross-agent-review, Test Framework, and MySpec.
...
- [x] 2.5 Express test-writing rules in MySpec artifacts only; do not create or modify `docs/rules/`.
...
- [x] 7.4 Run `myspec validate optimize-full-verification-runtime --strict --no-interactive` and `myspec validate --all --strict --no-interactive`.
