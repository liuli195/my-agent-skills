...
- [x] 以固定 Verification Baseline（验证基线）运行 Build and Verify（构建与验证）时，`checked`（已检查）非空、包含 `verify.my-spec` 且最终状态通过。
...
- Integration（集成）：固定基线 `309243c` 快速验证报告 `checked: verify.my-spec, verify.runtime-boundaries`、`full-not-run: true`、`status: passed`。
