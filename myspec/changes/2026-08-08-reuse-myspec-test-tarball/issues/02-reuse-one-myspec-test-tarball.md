...
- Review blockers（审查阻断）：首次模板实现使用默认 `copytree` 展开 Linux npm 符号链接，复制后的裸 `myspec` 会失去包内相对路径；每个 worker 会话结束未清理模板临时目录，且异常路径可能残留；现有测试仅单进程模拟一次安装，未证明 4 个真实 worker 各自只安装一次并运行独立复制品。
- Unresolved risks（未解决风险）：模板缓存只比较候选路径且未设置文件权限只读；当前 Interface（接口）不暴露模板、同一运行不替换候选文件，因此暂按审查判断处理。上述阻断修复后必须重新完成三次性能测量。
