...
- [ ] 目标 Git（版本管理）工作区或暂存区原本不干净时，迁移拒绝修改文件或提交。
- [ ] 新 CLI（命令行程序）快速验证成功后，仅识别的旧运行时被删除，且迁移只提交自身文件。
- [ ] 快速验证失败或旧运行时不符合识别范围时停止并保留现场。
- [ ] 迁移不扫描或改写任意既有调用方。
- [ ] Plugin Sync（插件同步）不再报告、刷新或提交 Build and Verify 运行时同步；无关市场、插件和用户文件保持不变。
