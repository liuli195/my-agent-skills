...
- [x] 本仓库受管自动化和用户入口改用新 CLI（命令行程序）及受控打包模块；调用方转换不改变既有构建、快速验证或完整验证选择语义。
- [x] Build and Verify、Plugin Sync（插件同步）和 Release Flow（发布流程）的运行时快照契约被最小替换，没有并行旧入口。
- [x] 与本次发布候选相关的验证保持快速冒烟，不新增 Tarball（npm 包文件）端到端回归。
- [x] 现有正式规格中与新行为冲突的要求同步更新。
- [x] 仓库的既有快速构建与验证入口通过。
