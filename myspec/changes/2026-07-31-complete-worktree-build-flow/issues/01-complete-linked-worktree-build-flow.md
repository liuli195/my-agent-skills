...
- [ ] 主工作区保存唯一的 `.venv` 和根级 `node_modules` 依赖实体；关联工作树通过目录链接共享两者，不重复安装大型依赖。
- [ ] 关联工作树在共享 Python 环境缺失、依赖指纹过期或依赖清单不一致时安全停止，并提示先初始化主工作区。
- [ ] 关联工作树已有非目标 `.venv` 或 `node_modules` 时停止，不覆盖或删除现有目录。
- [ ] Windows 关联工作树流程在运行构建前激活其链接到共享环境的 `.venv`，配置中的子 Python 命令使用同一初始化环境。
