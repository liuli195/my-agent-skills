...
- [ ] 门禁 1：完成 Requirements（需求）通过后，获批需求产物发布并提交，再转入门禁 2：进入 Implementation（实施）。
- [ ] 门禁 2：进入 Implementation（实施）通过后开始实施；实施、验证和审查达到完成条件后转入门禁 3：进入 Delivery（交付）。
- [ ] 门禁 3：进入 Delivery（交付）通过后，才能开始正式规格处理。
- [ ] 门禁 4：授权 PR（拉取请求）交付只有在正式 MySpec（自有规格）差异获批、应用并校验后才能通过。
- [ ] 门禁 4：授权 PR 交付没有下一正式门禁，流程最终完成仍由 PR 实际合并、目标分支同步和安全清理判定。
...
- [ ] 新 Pi（编码代理）进程真实冒烟证明缺少门禁 2：进入 Implementation（实施）的授权时停止、报告恢复入口且 Git（版本管理）状态不变。
