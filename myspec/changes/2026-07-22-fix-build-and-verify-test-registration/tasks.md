...
- [x] 3.1 运行定向回归测试、完整 pytest（Python 测试运行器）套件、MySpec（自有规格）严格校验及 Build and Verify（构建与验证）的 build（构建检查）和默认 verify（快速验证）。
...
- 范围决定：用户明确选择 Hotfix（热修复）流程；改动仅补齐既有配置清单，不涉及接口、架构或配置结构变更。