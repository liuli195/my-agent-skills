...
- [x] 3.3 运行 MySpec（自有规格）严格校验并复核规格场景与任务完成状态。
...
- `myspec validate configure-build-and-verify-init-script --strict`（严格规格校验）：通过。
- Build and Verify（构建与验证）的 build（构建检查）和默认 verify（快速验证）：通过。
- 升级判定：用户选择继续 Tweak（小改流程）；15 个变更文件主要为 MySpec（自有规格）和 Comet（彗星流程）元数据，未涉及跨模块架构变更。
