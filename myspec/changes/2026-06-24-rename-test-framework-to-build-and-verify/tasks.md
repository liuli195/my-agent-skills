...
- [x] 1.5 Keep the MySpec（自有规格） change modeled as a rename（改名） of the existing `test-framework-plugin`（测试框架插件） capability, not as a delete/add capability replacement.
...
- [x] 3.4 Update MySpec（自有规格） main specs and non-archived docs that describe current active commands.
...
- [x] 5.5 Run `myspec validate rename-test-framework-to-build-and-verify --strict`.
