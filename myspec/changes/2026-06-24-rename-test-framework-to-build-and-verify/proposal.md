...
- 更新本仓库 Comet（双星流程）、PR Flow（拉取请求流程）、marketplace（市场目录）、release projection（发布投影）、MySpec（自有规格）和测试引用到新入口。
...
- 影响 `tests/test_test_framework_plugin.py`、`tests/test_local_plugin_build_checks.py`、`tests/test_pr_flow_cli.py`、插件 package（包）测试和相关 MySpec（自有规格）测试。
