...
3. fast（快速验证）是框架执行模式，不是目标仓库配置能力。
   ...
   - 本仓库实际配置把 `verify.checks` 拆成 `verify.local-build-contract`、`verify.agent-guard`、`verify.release-flow`、`verify.pr-flow`、`verify.cross-agent-review`、`verify.test-framework` 和 `verify.myspec`；`verify --full` 的语义是运行这 7 个检查项的并集，默认 `verify` 只按 changed files（变更文件）命中相关检查项。
