...
- 再用最小实现让测试通过。
- 最后运行聚焦测试和 `myspec validate`（开放规格校验）。

## Spec Patch

已回写 `myspec/changes/fix-pr-flow-recovery-paths/specs/pr-flow-plugin/spec.md`（拉取请求流程插件增量规格），并补充 EOF（连接提前结束）耗尽状态、checks（检查）等待阻断处理和 closing references（关闭引用）追加位置。
