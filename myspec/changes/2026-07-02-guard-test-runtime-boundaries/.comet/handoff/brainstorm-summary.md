...
回写 MySpec（自有规格）design/spec/tasks（设计/规格/任务）：统一 Python AST（Python 语法树）扫描，明确 helper（辅助函数）/ fixture（测试夹具）边界，明确 E2E allowlist（端到端白名单）身份格式，加入 full verification（完整验证）和 before/after runtime（前后运行时间）记录。
