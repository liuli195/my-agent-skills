...
#### Scenario: 用户明确指定时使用网页 ChatGPT

- **WHEN** 主 Agent（代理）委派 Architect（架构师）任务且用户明确指定网页 ChatGPT
- **THEN** 使用宿主提供的 `awehitch/SKILL.md`（技能）路径；网页 ChatGPT 不可用时回退到本地具名 Architect（架构师），具名代理不可用时再按通用代理回退规则处理，并声明回退
