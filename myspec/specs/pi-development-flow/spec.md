...
- **THEN** 系统在 Gate 3 — Specification Archival and Delivery 中同时展示正式规格内容摘要、规格变更数量（新增、删除；存在时列出修改、改名）、验证证据、已知风险、准确交付动作和预期结果，并等待一次确认；该确认同时授权正式规格应用、校验和交付，确认后自动连续执行，交付成功结束后才进入 Completion Check（完成检查），交付中断状态保留为 Gate 3 的未完成恢复位置；完整正式规格差异通过引用提供，不在门禁输出中展开

#### Scenario: 一次确认并执行交付

- **WHEN** Gate 3 — Specification Archival and Delivery 已在同一输出中展示正式规格内容摘要、规格变更数量和交付动作
- **THEN** 用户的一次确认同时授权正式规格差异和交付动作；系统随后应用并校验正式规格差异，自动执行已确认的交付动作，不再请求第二次确认；只有交付成功结束后才转入 Completion Check
...
系统 MUST 为三个正式门禁和 Gate 3 之后的 Completion Check（完成检查）使用固定标题及以下四个固定区块，且顺序不可改变：`状态与待确认`、`核心内容摘要`、`引用`、`下一步`。核心摘要 MUST 只展示当前门禁的判断重点；普通的完整计划、差异和证据 MUST 通过引用提供；Gate 3 的核心摘要 MUST 展示正式规格内容摘要、规格变更数量（新增、删除；存在时列出修改、改名）和准确交付动作，但 MUST NOT 展开完整正式规格差异，完整差异通过引用提供；Completion Check 不是第四个正式授权门禁。
...
- **THEN** Development Flow（开发流程）在同一输出中展示正式规格内容摘要、规格变更数量和交付动作，并以一次确认同时授权两者；完整正式规格差异通过引用提供，系统不改写正式规格差异内容、不重复提问
