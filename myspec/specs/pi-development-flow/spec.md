...
系统 MUST 在需求讨论开始时先完成与流程等级相称的只读架构分析；系统验证分析结果；若结果错误、不完整、不可接受、无法验证或来源不支持，MUST 停止并报告阻塞，不得展示或使用整体方向；验证并接受后，MUST 将其作为整体方案、后续需求讨论的设计依据和证据；用户确认整体方向后，才按待解决决策是否能在一个会话内厘清选择需求讨论方式，并在用户确认测试接缝、规格和纵向票据后，才发布已确认的领域术语与需求产物。整体方向确认不是新的正式门禁，且不发布产物或授权实施、交付。

#### Scenario: 先确认整体方向

- **WHEN** 用户为新的 Development Flow（开发流程）开始需求讨论
- **THEN** 系统先完成与流程等级相称的只读架构分析；分析返回来源位置和不确定性，系统验证并接受后才以该结果作为后续需求讨论的设计依据和证据；用户以正常对话确认后，系统才继续讨论未决细节，并在生成规格和票据前确认整体分析、验证和需求讨论材料已经完成
...
- **THEN** 系统重新完成只读架构分析，验证并接受结果后返回整体方案并再次确认方向；其他答案继续处理未决细节
...
系统 MUST 在实施审查及验证通过后完成正式规格存档，并在 Gate 3 — Specification Archival and Delivery（规格存档并交付）自身的结果中请求一次确认，由该确认同时授权正式规格应用和交付；随后完成交付检查、结果合并、目标结果同步和非强制安全清理。Development Flow（开发流程）MUST 默认排除本地安装、客户端同步、市场刷新和发布，除非用户明确请求某个具体动作。
...
- **THEN** 系统在 Gate 3 — Specification Archival and Delivery 自身结果的四个固定区块中展示正式规格内容摘要、规格变更数量（新增、删除；存在时列出修改、改名）、验证证据、已知风险、准确交付动作和预期结果，并在该结果中请求一次确认；该确认同时授权正式规格应用、校验和交付，确认后自动连续执行，交付成功结束后才进入 Completion Check（完成检查），交付中断状态保留为 Gate 3 的未完成恢复位置；完整正式规格差异通过引用提供，不在门禁输出中展开

#### Scenario: 一次确认并执行交付

- **WHEN** Gate 3 — Specification Archival and Delivery 自身结果已展示正式规格内容摘要、规格变更数量和交付动作
- **THEN** 用户在该 Gate 3 结果中的一次确认同时授权正式规格差异和交付动作；系统随后应用并校验正式规格差异，自动执行已确认的交付动作，不再请求第二次确认；只有交付成功结束后才转入 Completion Check
...
系统 MUST 为三个正式门禁和 Gate 3 之后的 Completion Check（完成检查）使用固定标题及以下四个固定区块，且顺序不可改变：`状态与待确认`、`核心内容摘要`、`引用`、`下一步`。核心摘要 MUST 只展示当前门禁的判断重点；普通的完整计划、差异和证据 MUST 通过引用提供；Gate 3 自身结果的核心摘要 MUST 展示正式规格内容摘要、规格变更数量（新增、删除；存在时列出修改、改名）和准确交付动作，但 MUST NOT 展开完整正式规格差异，完整差异通过引用提供；Gate 3 的结果不得与其他门禁或 Completion Check 合并；Completion Check 不是第四个正式授权门禁。
...
- **WHEN** Gate 3 — Specification Archival and Delivery（规格存档并交付）自身结果需要确认正式规格差异和交付动作
- **THEN** Development Flow（开发流程）在 Gate 3 自身结果的四个固定区块中展示正式规格内容摘要、规格变更数量和交付动作，并以该结果中的一次确认同时授权两者；完整正式规格差异通过引用提供，系统不改写正式规格差异内容、不重复提问
