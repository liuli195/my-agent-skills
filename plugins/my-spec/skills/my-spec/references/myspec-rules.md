...
开发模式的裸 `myspec` 是机器级单一源码绑定，不会按当前目录自动切换。切换到目标工作树前先运行：

```text
myspec init --dev --source <目标工作树>
myspec doctor
```

`doctor`（诊断）中的 `binding`（绑定）报告会列出源码工作树、源码提交、目标工作树、目标提交和 `worktreeMatch`（工作树匹配结果）。如果结果不匹配，`apply-delta`（应用增量规格）会在预览、备份或替换规格前停止；修复方式是按目标工作树重新执行上述绑定切换。单一全局绑定只支持多个工作树串行应用，不支持并行写入；各工作树的 `.local/spec-work/` 锁不能把全局源码绑定变成独立运行时。
