...
6. 状态为 `WAITING_DECISION` 时禁止重新扫描获取下一项。调用 `state-current` 一次只展示一条候选、来源证据、原因和推荐答案，通过 `state-decide` 保存决定和推进游标，并用 `state-status` 报告稳定的总数和剩余数；禁止批量接受。
7. 规格为空时，为确定项生成初始 Delta（增量规格）；已有规格时只生成相对变化。没有明确证据不得推断删除或改名。
8. 没有未决项后用 `spec_ops.py` 校验 Delta（增量规格），生成并校验预览，展示完整差异并等待最终确认。
9. 确认后重查主规格指纹并原子应用；最终校验失败时恢复原规格。
