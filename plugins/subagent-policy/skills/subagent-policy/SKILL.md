...
- 每个会话在首次委派前检查一次有效宿主配置。选择角色前必须能读取宿主活跃配置、模型注册表和适用于本契约的原生映射。
- 只启用下列四个角色；每个角色的描述、模型、思考强度、能力/资源、`prompt_mode: append`（追加提示词模式）和提示词必须完全匹配。缺少值、无法证明、存在项目覆盖、模型解析失败或任何差异，都停止，不修复、不覆盖、不继承、不降级、不改用其他角色。
- 本 Skill（技能）只提供 Pi（编码代理）映射；Implementer（实施者）还必须证明 `extensions: false`（禁用扩展）和 `skills: tdd`（预加载测试驱动开发技能）。

## 流程编排

1. 在选择角色前，确认默认代理已禁用、没有未登记角色，并且只启用四个固定角色；确认每个角色完整配置和 `prompt_mode: append`。完成条件是四角色配置逐字段与契约一致。
2. 逐项确认固定模型能在宿主活跃模型注册表中解析，并确认宿主具有适用于本契约的原生映射。仅提供下表所列 Pi 映射；任一模型、映射或配置无法证明，立即在委派前停止。完成条件是四个固定模型均可解析且宿主映射已经验证。
...
4. 只有每个字段都完全匹配时，才允许选择相应的宿主原生角色。不得临时覆盖模型、思考强度、能力或提示词；如果任一字段不同、无法证明，或者宿主没有经过验证的 host Adapter（宿主适配器），则在委派前停止。完成条件是委派入口、角色、模型、资源和适配器均有证据。
5. 子代理返回报告只能作为线索，不能证明任务完成。主代理在依赖其结果或宣告工作完成前，必须验证子代理的实际结果。并核验角色与运行时元数据、文件、分支、差异、提交、工作树和检查结果。完成条件是实际结果与固定契约、任务范围和验证证据一致。
