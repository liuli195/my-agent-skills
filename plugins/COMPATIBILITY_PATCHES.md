...
- **状态**：上游已修复，本地补丁已由 `0.1.8` 官方版本替代，无需继续保留
...
- **上游修复确认**：`0.1.8` 在启动 MCP（模型上下文协议）连接前调用 `waitForDaemonStatus`，主动确认 Windows（视窗系统）命名管道已可用；未套用本地补丁的真实连接验证成功并发现 50 个工具。
- **后续处理**：不再保留或重新应用原 1.5 秒等待补丁；只有未来版本重新出现同一问题时，才按新版本行为重新诊断。
