...
| CodeQL security check（CodeQL 安全检查） | 开启时输出 GitHub Rulesets（GitHub 规则集）远端待办：配置 `Require code scanning results`（要求代码扫描结果），并选择 `CodeQL` 作为 code scanning tool（代码扫描工具）。 |
