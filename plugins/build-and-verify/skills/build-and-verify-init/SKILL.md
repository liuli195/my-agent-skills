...
3. 生成草案前，读取 `references/config-draft.md`（配置草案规则），生成 `build.checks`（构建检查项）和 `verify.checks`（验证检查项）。
...
7. 写入后按 `references/validation.md`（校验规则）执行 config（配置）结构校验。

## Output（输出）

- 写入前输出候选 checks（检查项）、`paths`（受影响路径）、自动推导的 `inputs`（缓存输入）、运行参数、覆盖摘要、默认备份路径、targeted dependency checks（定向依赖检查）结果和 environment checks（环境检查）结果。
