...
## Dynamic Scan（动态扫描）与 Candidate Probe（候选命令探测）

- 动态扫描命令的实际读取范围必须同时覆盖 paths（受影响路径）和 inputs（缓存输入）。
- 若完整范围会让复合检查在不相关变更时运行，应建议拆分为专用检查，而不是扩大复合检查。
- 先以静态证据确定范围；只有静态证据不足、候选命令探测能消除配置不确定性且成本可接受时，才建议探测。
- 不得在未确认时运行候选 command（命令）。确认前展示完整命令组、总成本和可能副作用；用户可一次确认该组探测。
- 探测执行前后核对 Git（版本管理）可见改动。发现改动时立即停止后续建议和写入，保留现场，不自动清理或恢复。
