...
validation（校验）必须在 `build-and-verify-init`（构建与验证初始化）插件内完成，不得把校验结果留到插件外部流程补做。写入前摘要必须同时列出 targeted dependency checks（定向依赖检查）结果和 environment checks（环境检查）结果；写入后必须立即报告 config（配置）结构校验结果。
