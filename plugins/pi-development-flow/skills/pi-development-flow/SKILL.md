...
4. **Implement.** After authorization, read and complete [implementation](references/implementation.md). This phase is complete when every ticket is integrated, its behavior evidence is recorded, the final user-entry smoke and fast verification pass, and the bounded overall review has no blocking finding.
