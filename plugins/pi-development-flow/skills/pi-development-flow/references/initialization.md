...
- the required Requirements（需求）Skills（技能）—`codebase-design`, `grill-with-docs`, its required `grilling` and `domain-modeling`, `to-spec`, and `to-tickets`—plus TDD（测试驱动开发）, review, MySpec（自有规格）, PR Flow（拉取请求流程）, Build and Verify（构建与验证）, and Pi Subagent Policy（Pi 子代理策略）are available; the `codebase-design` Skill（技能） is discoverable and readable by the effective `Architect`（架构师） role, and `pi-subagent-policy` is available to validate and route that delegation; `wayfinder` is required only when the selected route needs its multi-session planning;
