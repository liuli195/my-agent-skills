...
- Gate 3：正式规格内容摘要、规格变更数量、校验结果、已知风险和准确交付动作。
- Completion Check：完成条件、实际状态和清理残留。

普通长内容通过“引用”提供入口；Gate 3 自身的四块结果中的“核心内容摘要” MUST 展示正式规格内容摘要和规格变更数量（新增、删除；存在时列出修改、改名），不展开完整正式规格。完整差异通过引用提供，准确交付动作仍须直接展示在 Gate 3 自身的“核心内容摘要”中；Gate 3 的结果不得与其他门禁或 Completion Check 合并。摘要保留作出当前判断所需的重点。
