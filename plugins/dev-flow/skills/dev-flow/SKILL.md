---
name: dev-flow
description: 在同一个 Git（版本管理）工作树和非 main（主干）功能分支中编排一项开发变更；当变更必须依次经过需求、串行或并行实施、验证、审查和 PR（拉取请求）交付时使用。
---
...
- 严格禁止跳过门禁：必须先向用户显式展示当前门禁的完整内容，再取得用户对该门禁的显式确认。此前的方案批准、启动流程指令或笼统授权均不能替代展示后的确认；缺少任一步都必须停留在当前门禁，不得执行门禁后的动作。
...
1. 只读核对入口前提，确认仓库、当前工作树、当前或拟新建分支和固定基线具备可复核证据。
...
5. 全流程只有两个正式确认。每个门禁都必须先向用户显式展示当前门禁的完整内容，再取得用户对该门禁的显式确认；此前对方案或启动流程的批准不能代替门禁确认。未确认时停留当前门禁，不执行门禁后的动作；语义不明确时停留澄清。每次确认在对应动作及失败恢复期间持续有效，需求或测试接缝改变时返回需求路由。
