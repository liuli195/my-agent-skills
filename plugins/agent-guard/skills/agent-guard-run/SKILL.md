...
5. 关闭实例：需要关闭 Guard Instance（守卫实例）时，读取 `references/close.md`，运行 Plugin Runtime（插件运行时）的 `close-instance` 命令。
6. Global Command Guard（全局命令守卫）拦截和排障按 `references/events.md` 处理。
7. 运行 `../agent-guard/scripts/run_guard_event.py` 并报告状态、审计位置和下一步。
