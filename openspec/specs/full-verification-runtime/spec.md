...
Full repository verification SHALL（必须）complete in under 60 seconds on the local development machine while preserving the existing behavior coverage. The current full verification command for this repository SHALL（必须）be `python plugins/build-and-verify/skills/build-and-verify/scripts/build_and_verify.py verify --project . --full` unless a later OpenSpec（规格流程）change explicitly replaces it.
...
- **THEN** the Build and Verify（构建与验证）runner MUST coordinate parallel execution for all configured verify checks（验证检查项）where safe
