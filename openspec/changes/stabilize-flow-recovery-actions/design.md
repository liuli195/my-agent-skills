...
现有 PR Flow（拉取请求流程）已经有多个局部恢复路径，例如缺 upstream（上游分支）、safe auto-push（安全自动推送）、EOF（连接提前结束）重试和 ruleset（规则集）等待。但这些路径的 reason（原因）、stop state（停止状态）和恢复动作分散在不同函数里。Release Flow（发布流程）preflight（预检）中本 change（变更）列出的失败也只输出错误列表，缺少稳定的下一步出口。
...
- 给 Release Flow preflight（发布预检）中本 change（变更）列出的失败补有序下一步提示。
...
2. Release Flow（发布流程）只在 preflight（预检）输出层补本 change（变更）列出的下一步。
