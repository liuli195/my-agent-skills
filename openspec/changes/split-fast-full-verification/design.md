...
2. 目标仓库初始化最小结构。
   ...
   - 初始化遇到已有 `scripts/check.py`、`.test-framework/config.json` 或 `.test-framework/.gitignore` 时必须拒绝覆盖并报告冲突，除非未来显式增加覆盖参数。
