...
- [x] 3.5 提供上下文值来源，例如捕获变量和当前 `git_head`。
- [x] 3.6 提供内置上下文值：`source_scope`、`profile_id`、`guard_id`、`effective_guard_id`、`runtime_scope`。

## 4. 运行时拦截

- [x] 4.1 添加 PreToolUse 测试，证明没有 Session Focus 时仍会执行全局命令守卫点。
- [x] 4.2 添加缺少 evidence（证据）、`head_ref` 过期、evidence 通过、缺少必需捕获值的测试。
- [x] 4.3 在 `route_pre_tool_use` 中先评估有效全局命令守卫集，再进入 Session Focus permission。
- [x] 4.4 复用共享 JSON 检查能力校验配置的 evidence。
- [x] 4.5 返回机器可读的拒绝输出，包含 `reason`、`next`、`suggestion`、匹配的有效守卫 ID 列表、失败守卫列表、捕获值和审计路径。
- [x] 4.6 审计记录必须区分全局命令守卫点和会话焦点权限检查。
- [x] 4.7 添加作用域测试：用户级安装或用户级静态规则在项目命令中默认使用 `.local/guard`；显式用户作用域使用 `~/.agents/guard`；Comet change review 不从用户级运行目录读取证据。
...
- [x] 5.1 验证没有全局命令守卫点匹配时，现有 Session Focus PreToolUse 行为不变。
- [x] 5.2 验证全局命令守卫点允许后，后续 Session Focus permission 仍可拒绝命令。
- [x] 5.3 更新 Agent Guard 运行文档，解释“会话焦点守卫”和“全局命令守卫点”的差异。
- [x] 5.4 更新目录和运行时文件约定文档，说明插件安装范围、静态画像范围、运行态数据范围的差异。
- [x] 5.5 运行聚焦的 Agent Guard runtime 和 validator 测试。
- [x] 5.6 运行仓库完整测试套件。
