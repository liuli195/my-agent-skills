...
- 写入前必须完成定向依赖检查和环境检查并展示结果；写入后必须执行配置结构校验。
...
如果未识别 Node（节点运行时）或 Python（Python 语言）生态，向导仍必须继续固定问答流程：让用户手动提供 build（构建检查）和 verify（验证）命令，再确认 `paths`（受影响路径）、`inputs`（缓存输入）、覆盖备份和配置校验。
...
覆盖前旧配置必须备份到 `.build-and-verify/backups/config-YYYYMMDD-HHMMSS.json`（备份配置文件）。如果 `.build-and-verify/backups`（备份目录）不存在，向导必须先创建该目录。向导还必须确保 `.build-and-verify/.gitignore`（忽略规则）包含 `/backups/`。
...
写入前还必须执行 environment checks（环境检查）：确认目标仓库路径存在且是目录、配置目录可创建或可写入；覆盖已有配置时，确认备份目录可创建且备份路径仍在目标仓库内。

写入后只执行配置结构校验，确认 `.build-and-verify/config.json`（配置文件）符合 runner（运行器）契约。初始化向导不自动执行仓库检查命令；完成条件只包括配置写入、配置结构校验、依赖/环境问题报告。
...
- [Risk] 定向依赖检查不等于真实环境完全可用。Mitigation: 写入前列出依赖和环境问题、允许用户在明确知情后写入，并提示用户可授权 agent（代理）协助处理环境和外部依赖问题。
...
4. 新增模板完整性测试，确认固定问答、覆盖备份、定向依赖检查、环境检查和配置校验规则存在。
