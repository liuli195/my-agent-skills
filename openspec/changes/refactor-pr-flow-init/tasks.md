...
- [ ] 1.1 Update `pr-flow-init` Skill（初始化技能） to describe agent（代理）问答、草案展示、校验、用户确认和写入边界。
- [ ] 1.2 Restructure `pr-flow-init` Skill（初始化技能） for progressive disclosure（渐进式披露）: hard boundaries（硬边界）, closed loop（闭环）, required flow（必需流程）, output（输出） and `references/`（参考文件） list only.
- [ ] 1.3 Add `references/questionnaire.md`（问答模板） with fixed questions, fixed options, selection consequences and jump rules.
...
- [ ] 1.5 Add `references/config-draft.md`（配置草案规则） for `.pr-flow/config.yaml`（配置文件） shape, defaults（默认配置）, branch overrides（分支覆盖） and `setup.github`（GitHub 配置建议）.
- [ ] 1.6 Add `references/validation.md`（校验规则） for dependency matrix（依赖矩阵）, validation output（校验输出） and write summary（写入摘要）.
- [ ] 1.7 Keep existing PR Flow（拉取请求流程） command boundaries in the Skill（技能） text: no commit（提交）, push（推送）, merge（合并）, or GitHub API（GitHub 接口） writes during init（初始化）.
...
- [ ] 4.4 Update package or Skill（技能） tests to ensure `pr-flow-init` documents agent（代理）问答 rather than script（脚本） terminal interaction.
- [ ] 4.5 Add tests that require `pr-flow-init/references/questionnaire.md`, `config-draft.md` and `validation.md`（问答模板、配置草案规则、校验规则） to exist and be referenced by the Skill（技能） entrypoint.
