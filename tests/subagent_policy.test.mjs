...
const contract = [
  ...
  "| Implementer（实施者） | 根据已确认需求修改代码或文档并验证结果。 | `gpt-6-luna` | `max`（最高） | 可写 |",
  "| Reviewer（审查者） | 根据需求和仓库规则独立审查代码或文档。 | `gpt-6-sol` | `high`（高） | 只读 |",
  "| Architect（架构师） | 调查架构、架构决策和疑难缺陷。 | `gpt-6-sol` | `high`（高） | 只读 |",
  ...
  "Architect（架构师）在用户明确指定时可以使用网页 ChatGPT",
