...
def run_extractor(input_path: Path, output_path: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, str(EXTRACTOR), str(input_path), "--output", str(output_path)],
        cwd=REPO_ROOT,
        text=True,
...
def test_generation_includes_guard_point_implementation_plan(tmp_path: Path) -> None:
    ...
    assert "先以 warn（警告）或 record（记录）模式验证" in implementation_plan
