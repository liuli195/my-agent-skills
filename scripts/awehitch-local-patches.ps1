...
$marker = 'PATCH(local): keep Codex registration project-local and validate fixed project config'
...
if (-not (Test-Path $codex) -or -not (Test-Path $cli)) { Row "补丁 4 项目级配置" "找不到文件" "未动" "缺 Codex 适配器文件" }
  ...
  if ($cxDone -and $clDone) { Row "补丁 4 项目级配置" "已处理" "跳过" "两个文件均有完整最终特征" }
    ...
    $canPatch=$true
    if ($cxLegacy) { $cx=$cx.Replace($legacyWrite,$aWrite).Replace($legacySig,$aSig).Replace($legacyCheck+"`n"+$aRet,$aRet) }
    elseif (-not $cxDone -and $cx.Contains($legacyMarker)) { $canPatch=$false }
    if ($clLegacy) { $cl=$cl.Replace($legacyCall,$aCall) }
