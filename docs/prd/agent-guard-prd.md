...
- MVP 要落在一个具体项目流程或 Skill 上，不能只停留在抽象生成器。
...
`agent-guard` 的用户入口优先是 Codex Skill 显式触发。用户可以通过 `/skills` 选择 `agent-guard`，也可以在提示中使用 `$agent-guard`，或使用“用 agent-guard 守护 pr-review-order”这类明确自然语言。无论哪种方式，都必须被解析为结构化激活请求，例如：

```yaml
action: activate_guard
guard_profile_id: pr-review-order
...
MVP 以“项目里的一个具体流程或 Skill”为首个落地对象。示例可以使用 `pr-review-order` 这类 PR 流程守卫，但 Runtime、目录结构和 Profile 模型必须保持通用，不把 PR 流程规则写入 Runtime。
...
- 自动迁移既有产物所有权。
- 全局多 Profile 裁决器。
- 完整 CI、完整测试目录和 verify fast/full 命令体系。
...
26. 作为项目维护者，我希望阻断模式必须显式启用，以便不会突然阻断正常工作。
...
41. 作为项目维护者，我希望无法解析明确守卫实例时不强阻断但必须审计，以便既避免误伤，也能 debug 守卫配置。

42. 作为项目维护者，我希望已解析实例但无匹配状态转换时默认忽略，以便无关 hook 事件不会制造噪音。

43. 作为项目维护者，我希望同一实例内多转换匹配被视为 Profile 错误，以便状态机不会在歧义下推进。

44. 作为项目维护者，我希望阻断只在明确守卫实例内部生效，以便多个 Guard Profile 或多个实例不会互相连坐。
...
阻断、警告、错误和允许结果的合并规则只适用于同一个守卫实例内部，不默认跨 Guard Profile 或跨守卫实例做全局合并。阻断必须绑定到明确守卫实例。
...
- 在提示中使用 `$agent-guard`。
- 使用“用 agent-guard 守护 pr-review-order”这类明确表达。
...
守卫简报是注入给 agent 的短文本或结构化摘要。它用于告诉 agent 当前守卫状态、允许下一步、禁止下一步、缺失产物、最近阻断原因和审计位置。

守卫简报用于提高执行效率，不替代 hook 的硬阻断。
...
守卫点是在某个事件或状态转换上执行的校验。守卫点可以是警告模式，也可以是阻断模式。
...
所有 adapter 必须输出统一标准事件 envelope。Guard Profile、状态机和守卫点只能依赖标准事件字段，不直接依赖原始 hook payload。原始 payload 保存为 `raw-event.json`，用于审计和 debug。
...
- 解析到 0 个实例：不强阻断，但必须写 `no_subject_match` 审计。
- 解析到多个实例：不强阻断，但必须写 `ambiguous_subject` 审计。
- 解析到 1 个实例：进入该实例的状态机。

`no_subject_match` 和 `ambiguous_subject` 审计至少包含原始事件引用、候选 Guard Profile、解析失败原因和修复建议。

阻断必须来自明确守卫实例，不能由全局 Runtime 对多个实例做连坐式阻断。
...
- 运行时读取当前状态。
- 用事件匹配可用转换。
- 如果没有匹配到转换，默认 `ignore`：不阻断、不审计、不推进状态。
- 如果 Guard Profile 显式配置 `unmatched_event_policy: audit` 或 `block`，才记录或阻断未匹配事件。
- 如果同一守卫实例内匹配到多个转换，视为 `ambiguous_transition`。
- `ambiguous_transition` 在阻断入口必须 block，在审计入口返回 error，必须写运行审计，且不得推进状态。
- 如果转换显式允许 `null` 或 `unknown`，才允许从未知状态开始。
- 执行转换绑定的守卫点。
- 只有全部阻断级守卫通过，才写入新状态。
- 警告级守卫失败只记录问题，不阻断状态推进，除非该转换配置为警告失败也不可推进。
- 每次匹配到唯一转换并执行守卫点时，必须写入审计记录。
- 已处于终止状态的守卫实例再次收到事件时，MVP 默认忽略，不阻断、不审计、不推进状态。
...
同一转换绑定多个守卫点时，结果按以下严重程度合并：

```text
error > block > warn > allow
```

`guard_error` 指守卫点自身运行失败，例如脚本异常、依赖缺失或输入产物读取失败。`guard_error` 默认按 block 处理，除非 Guard Profile 对该守卫点显式配置 `on_error: warn` 或 `ignore`。

`runtime_error` 指 Guard Runtime 自身错误，例如 Profile 加载失败、状态写入失败或锁获取异常。`runtime_error` 在阻断入口必须 block，在审计入口返回 error。
...
是否阻断、在哪个状态转换上阻断、失败后如何处理，都必须由 Guard Profile 的状态机和守卫点定义。

### Guard Injection

Guard Injection 是把当前守卫实例的状态、允许动作、禁止动作、缺失产物和下一步建议提前提供给 agent 的机制。它用于提高执行效率，不替代 Codex Hook 或 Git Hook 的硬阻断。
...
- block：阻断继续。
- error：运行时错误。
- no_subject_match：无法解析守卫实例，不强阻断但必须审计。
- ambiguous_subject：匹配到多个守卫实例，不强阻断但必须审计。
...
## MVP 示例：PR review 顺序守卫

MVP 可以用 `pr-review-order` 作为项目级流程守卫样例。该样例只用于验证架构，不把 PR 规则写入 Runtime。
...
守卫点: require_two_cross_reviews_complete
结果: block
状态: 不推进
审计: 写入 run 目录
...
- 显式激活 `pr-review-order` 后创建或匹配守卫实例。
- 未完成两个交叉 review 时，启动安全 review 会被阻断。
- 只完成一个交叉 review 时，启动安全 review 仍会被阻断。
- 两个交叉 review 完成后，允许启动安全 review。
- 未完成安全 review 时，`git pre-push` 被阻断。
- 安全 review 完成后，`git pre-push` 放行。
- 每次状态或缺失产物变化后，Guard Brief 更新。
- 相同 Guard Brief 不重复注入。

GitHub Actions、完整测试目录、verify fast/full 暂不进入初始化范围。
...
- Codex hook 用于过程观察、动态注入和可阻断入口上的硬阻断。
- Git hook 用于提交或推送前兜底。
