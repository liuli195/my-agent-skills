...
- `openspec/specs/`，尤其是和当前工作范围相关的 capability spec（能力规格）
- 需要历史追溯时，再读取 `openspec/changes/archive/2026-06-16-baseline-agent-guard-docs-migration/source-docs/`；归档不作为当前契约
...
如果输出内容和已有 OpenSpec spec（规格）冲突，需要明确指出冲突点，不要静默覆盖已有决定。
