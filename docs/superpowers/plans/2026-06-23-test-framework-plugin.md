...
初始化目标仓库的 `scripts/check.py`、`.test-framework/config.json` 和 `.test-framework/.gitignore`。
```
...
- Create: `plugins/test-framework/skills/test-framework/assets/templates/scripts/check.py`
...
def test_init_creates_standard_artifacts(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "scripts" / "check.py").is_file()
...
def test_init_refuses_existing_runner_conflict(tmp_path: Path) -> None:
    runner = tmp_path / "scripts" / "check.py"
    runner.parent.mkdir(parents=True)
    runner.write_text("print('existing')\n", encoding="utf-8")

    result = run_init(tmp_path)

    assert result.returncode == 1
    assert "existing_file" in result.stderr
    assert "scripts/check.py" in result.stderr.replace("\\", "/")
...
def test_initialized_repository_runs_build_verify_full_and_cache(tmp_path: Path) -> None:
    ...
    build = subprocess.run([sys.executable, "scripts/check.py", "build"], cwd=tmp_path, text=True, capture_output=True, check=False)
    fast = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
    cached = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
    full = subprocess.run([sys.executable, "scripts/check.py", "verify", "--full"], cwd=tmp_path, text=True, capture_output=True, check=False)
...
def test_initialized_repository_does_not_cache_failed_results_or_run_full_on_miss(tmp_path: Path) -> None:
    ...
    first = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
    second = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
...
def test_initialized_repository_default_verify_collects_all_worktree_change_kinds(tmp_path: Path) -> None:
    ...
    unstaged = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
    assert unstaged.returncode == 0

    tracked.write_text("staged\n", encoding="utf-8")
    subprocess.run(["git", "add", "src/tracked.txt"], cwd=tmp_path, text=True, capture_output=True, check=False)
    staged = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
    assert staged.returncode == 0

    (tmp_path / "src" / "new.txt").write_text("untracked\n", encoding="utf-8")
    untracked = subprocess.run([sys.executable, "scripts/check.py", "verify"], cwd=tmp_path, text=True, capture_output=True, check=False)
...
def planned_copies(root: Path) -> tuple[tuple[Path, Path], ...]:
    return (
        (TEMPLATE_ROOT / "scripts" / "check.py", root / "scripts" / "check.py"),
...
The implementation should use the same helper names planned for repository `scripts/check.py`: `_load_config`, `_changed_files`, `_selected_checks`, `_cache_key`, `_cache_load`, `_cache_store`, `_run_check`, `run_build`, and `run_verify`.
