...
- [ ] **Step 1: Write failing tests（写失败测试）**
...
- [ ] **Step 2: Run tests to verify failure（运行测试确认失败）**
...
- [ ] **Step 1: Add shared recovery helpers（添加共享恢复辅助函数）**
...
- [ ] **Step 2: Route status by classified reason（按分类后的原因返回状态）**
...
- [ ] **Step 3: Add action to checks and ruleset stops（给检查和规则集停止补动作）**
...
- [ ] **Step 4: Make `--fixes None` actionable（让修复问题编号为空可恢复）**
...
- [ ] **Step 5: Run PR Flow tests（运行拉取请求流程测试）**
...
- [ ] **Step 3: Add output-only formatter（添加只影响输出的格式化函数）**
...
- [ ] **Step 4: Run Release Flow tests（运行发布流程测试）**
...
- [ ] **Step 1: Write guard test（写防回归测试）**
...
- [ ] **Step 1: Run focused CLI tests（运行聚焦命令行测试）**
...
- [ ] **Step 2: Run fast repository verification（运行快速仓库验证）**
...
- [ ] **Step 1: Run PR Flow user entrypoint regression（运行拉取请求流程用户入口回归）**
...
- [ ] **Step 2: Run Release Flow preflight and publish shape regression（运行发布流程预检和发布形态回归）**
...
- [ ] **Step 3: Final status check（最终状态检查）**
...
- [ ] **Step 4: Final commit if verification changed files（如验证后还有文件变更则提交）**
