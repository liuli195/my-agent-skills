...
**Architecture（架构）:** 保持现有 `build-and-verify`（构建与验证）Skill（技能）和命令行 `init`（初始化）不变；新增一个只负责对话式初始化的 Skill（技能）目录。所有可变细节放入 `references/`（参考文件）并由测试锁定：问答、生态识别、配置草案、校验和试运行。
...
- Create（新建）: `plugins/build-and-verify/skills/build-and-verify-init/references/validation.md`，定向依赖检查、结构校验和 dry run（试运行）规则。
...
- 选择后果：
  ...
  - 仅展示草案：不写入文件。
  - 停止初始化：保留已有配置。
- 跳转规则：选择覆盖进入 Q9；选择仅展示草案时输出草案并结束。

## Q9 备份路径确认

- 固定选项：
  - 使用默认备份路径 `.build-and-verify/backups/config-YYYYMMDD-HHMMSS.json`。
  - 停止初始化。
- 选择后果：
  - 使用默认备份路径：写入前复制旧配置，并确保 `.build-and-verify/.gitignore`（忽略规则）包含 `/backups/`。
  - 停止初始化：不覆盖旧配置。
- 跳转规则：确认后进入 Q10。
...
- 固定选项：
  - 写入后只运行 `build`（构建检查）。
  - 写入后只运行默认 `verify`（快速验证）。
  - 写入后运行 `build`（构建检查）和默认 `verify`（快速验证）。
  - 写入后运行 `verify --full`（完整验证）。
  - 不运行 dry run（试运行），只做 config（配置）结构校验。
  - 返回 Q7。
  - 停止初始化。
- 选择后果：
  - `build`：只执行构建检查范围。
  - 默认 `verify`：只执行快速验证范围。
  - 两者都运行：先构建检查，再快速验证。
  - `verify --full`：成本更高，必须先说明原因和耗时风险。
  - 不运行 dry run：仍执行配置结构校验。
- 跳转规则：确认后进入 Q11。
...
- 固定选项：
  ...
  - 取消，不写入。
- 选择后果：
  - 确认写入：先展示 targeted dependency checks（定向依赖检查）结果；如用户继续，则备份旧配置、写入新配置、做结构校验并执行选择的 dry run（试运行）。
  - 返回 Q10：重新选择试运行范围。
  - 取消：不写入配置。
- 跳转规则：确认写入前必须展示完整草案、覆盖摘要、备份路径、依赖检查结果和 dry run（试运行）范围。
```
...
- `check` 或 `verify` -> `verify.node-check`
...
dry run（试运行）范围必须由用户选择，只能使用现有公开命令范围：
...
- Spec coverage（规格覆盖）: 计划覆盖双 Skill（技能）入口、固定问答、progressive disclosure（渐进式披露）reference（参考文件）、Node（节点运行时）/Python（Python 语言）识别、未识别生态分支、配置草案、覆盖备份、定向依赖检查、结构校验、dry run（试运行）范围和命令行 `init`（初始化）不变。
