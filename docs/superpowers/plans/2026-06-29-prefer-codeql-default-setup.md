...
- [ ] **Step 1: 更新 questionnaire（问答模板）断言**
...
- [ ] **Step 2: 更新 config draft（配置草案）和 validation（校验说明）断言**
...
- [ ] **Step 4: 增加 validate（校验）不调用 gh（GitHub 命令行工具）的测试**
...
- [ ] **Step 1: 更新 CodeQL security check（CodeQL 安全检查）场景**
...
- [ ] **Step 2: 更新 PR status checks（拉取请求状态检查）场景**
...
- [ ] **Step 3: 更新 config draft（配置草案）和 validation（校验说明）**
...
- [ ] **Step 1: 删除本地 workflow（工作流）影响 CodeQL（代码查询扫描）待办的逻辑**
...
- [ ] **Step 3: pr-flow-init（拉取请求流程初始化）到 validate/init（校验/初始化）写入端到端回归**
...
不运行 `git commit`（提交）、`git push`（推送）、不创建 PR（拉取请求）。如需要提交，先取得用户明确授权。
