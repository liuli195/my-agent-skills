...
- [x] **Step 1: 写失败测试，要求入口引用三个 references（参考文件）且不内联完整问答**
...
- [x] **Step 4: 创建三个 references（参考文件）的最小骨架**
...
- [x] **Step 1: 写 validate（校验）成功测试，确认只读且不调用 GitHub API（GitHub 接口）**
...
- [x] **Step 1: 写 confirmed config（已确认配置）写入测试**
...
- [x] **Step 3: 写 validate error（错误）阻止写入测试**
...
- [x] **Step 6: 更新测试 helper（辅助函数）摆脱旧默认 init（初始化）**
...
- [x] **Step 1: 运行 PR Flow（拉取请求流程）聚焦测试**
...
- [x] **Step 3: 若 OpenSpec（开放规格）命令不可用，记录替代验证**
...
- [x] **Step 5: Final commit（最终提交） if previous tasks left verification-only changes**
...
- [x] Spec（规格）覆盖：计划覆盖 `pr-flow-init` Skill（初始化技能）渐进式披露、三个 references（参考文件）、只读 validate（校验）、validate error（错误）阻止 init（初始化）写入、已确认配置写入、旧默认 init（初始化）拒绝、`setup.github`（GitHub 配置建议）不被运行命令消费、依赖矩阵、端到端回归。
- [x] 无新依赖：只用现有 Python（编程语言）、argparse（命令行参数解析）、PyYAML（YAML 配置读取）和 pytest（测试框架）。
