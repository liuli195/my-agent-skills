...
- `cross-agent-review`（跨代理审查）只生成 Markdown（标记文本）审查报告，由主 agent（主代理）判断严重级别并显式写入 Guard（守卫）证据。
- Agent Guard（代理守卫）统一使用 `.local/guard/evidence/` 默认目录保存由主 agent（主代理）生成的 pass marker（通过标记）。
...
### cross-agent-review（跨代理审查）和 Guard（守卫）证据路径

本次讨论把 PR Flow init（拉取请求流程初始化）后续审查证据也收敛到更简单的职责划分：

- `cross-agent-review run`（跨代理审查运行）只聚合 reviewer（审查代理）的 Markdown（标记文本）结果并写 `review-report.md`（审查报告）。
- reviewer（审查代理）必须为每个 finding（发现项）写 `Severity: CRITICAL|IMPORTANT|WARNING|SUGGESTION`（严重级别）。
- 脚本不解析 finding（发现项）、不去重、不判断是否通过。
- 主 agent（主代理）读取报告并确认没有未处理 CRITICAL（严重阻断）或 IMPORTANT（重要阻断）后，才运行 `mark-pass`（标记通过）。
- `mark-pass`（标记通过）写入 `.local/guard/evidence/<profile_id>/cross_agent_review_pass/<change>/<head_ref_short>/pass.json`。
- Agent Guard（代理守卫）只按 `artifacts.yaml`（产物注册文件）读取和校验 guard-defined evidence（守卫定义证据），不实现 cross-agent-review（跨代理审查）内部流程。

Timeout（超时）归属保持在插件内部：单 reviewer（审查代理）480 秒、整体 SDK dispatch（开发包派发）540 秒。主 agent（主代理）不得再在外层包一个短于 540 秒的 timeout/watchdog（超时/看门等待）。
...
- cross-agent-review（跨代理审查）Markdown（标记文本）报告、严重级别和 `mark-pass`（标记通过）职责划分。
- Agent Guard（代理守卫）guard-defined evidence（守卫定义证据）默认路径和 Comet review gate（彗星审查门禁）产物注册。
