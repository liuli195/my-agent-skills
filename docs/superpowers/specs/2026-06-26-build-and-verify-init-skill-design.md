...
未识别生态时，向导仍继续固定问答流程：让用户手动提供 build（构建检查）和 verify（验证）命令，再确认 paths（受影响路径）、inputs（缓存输入）、覆盖备份和配置校验。
...
- 新增 reference（参考文件）完整性测试：
  - `questionnaire.md`（问答模板）包含 10 个固定问题、固定选项和最终写入确认。
...
- 新增模板执行模拟测试：按 reference（参考文件）的关键规则生成配置、备份已有配置、报告定向依赖和环境问题、校验结构，用于证明 10 步模板可以产出符合 runner（运行器）契约的 `.build-and-verify/config.json`（配置文件）。
