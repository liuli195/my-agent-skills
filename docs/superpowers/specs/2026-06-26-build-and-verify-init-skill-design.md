...
- 新增模板执行模拟测试：按 reference（参考文件）的关键规则生成配置、备份已有配置、报告定向依赖问题、校验结构并执行用户选择范围的 dry run（试运行），用于证明 11 步模板可以产出符合 runner（运行器）契约的 `.build-and-verify/config.json`（配置文件）。
...
- 风险：轻量 Skill（技能）依赖 agent（代理）按模板执行，仍可能漏步骤。
  缓解：用测试检查 reference（参考文件）存在、关键规则不可缺，并用模板执行模拟覆盖备份、结构校验、定向依赖检查和 dry run（试运行）主流程。
