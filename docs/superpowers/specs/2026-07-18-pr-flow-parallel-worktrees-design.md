...
扩展现有 `init_complete_project` 裸仓库夹具，创建两个关联工作树、两个源分支和各自独立的假 GitHub（代码托管平台）响应。使用独立子进程运行 PR Flow（拉取请求流程），并用事件文件确定性地控制并发时点，不依赖固定等待时间。

端到端场景覆盖：

1. 两个工作树分别推进不同 PR（拉取请求），状态和锁互不覆盖；
2. 一个流程先推进远端目标，另一个在远端修改前进入 `base_outdated`；
3. 一个流程失败或持锁时，另一个工作树继续；
4. 目标冲突只停止当前流程；
5. 源分支被另一工作树占用时，远端和本地源分支均不删除；
6. cleanup（清理）安全收尾及显式工作树删除。
