...
- 不查询 GitHub Branch Protection（分支保护）状态来决定 cleanup（清理）；cleanup 只保证不删除 base branch（目标分支）。
...
- `DISPATCH_REQUIRED`：checks 未完成、需要外部系统启动，或 PR 仍是 draft（草稿）需要转 ready（可审查）。
- `REPLY_OR_FIX_REQUIRED`：checks 或 review gate 阻塞。
- `EXCEPTION_REQUIRED`：状态异常，不能安全判断下一步。

当没有 stop state 时，`diagnose` 输出 `ready` 并返回成功，同时写入 `nextCommand: complete`。这样外部 checks 完成后，agent 可以再次运行 `diagnose` 得到明确的继续路径，而不是落入异常状态。
...
cleanup 不查询 GitHub Branch Protection 或 Rulesets。首版边界是本地和远端 Git 状态安全：只删除 PR 的 head branch，切回并同步 base branch，不删除 base branch。
...
- authorization phrase 配置存在且算法受支持。
...
顺序上，hotfix 先检查 authorization phrase 配置是否可用，再运行 `hotfix.verifyCommand`，最后校验用户输入的 authorization phrase。这样避免未配置授权时运行验证命令，同时保留“验证通过后再确认”的交互语义。
...
- Diagnose 测试：覆盖四个 stop state、draft PR（草稿拉取请求）和 ready（就绪）状态。
