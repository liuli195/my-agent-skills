...
- 初始化产物：目标仓库生成 `scripts/check.py`、`.test-framework/config.json`、`.test-framework/.gitignore`。
...
- 端到端初始化：在临时仓库运行 `test-framework init` 后实际执行 `python scripts/check.py build`、`python scripts/check.py verify` 和 `python scripts/check.py verify --full`。
