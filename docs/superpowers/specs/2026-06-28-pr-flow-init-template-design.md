...
- `config-draft.md`（配置草案规则）展示用户可读摘要，禁止展示完整 YAML（配置格式）草案。
...
- 草案必须给用户可读摘要，并禁止展示完整 YAML（配置格式）草案。
