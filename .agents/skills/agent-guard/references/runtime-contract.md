...
工具调用成功后的状态变化不得由 Hook（钩子）自动触发。主 agent（主代理）必须在完成对应状态后，主动提交 `event_type=state_completed` 和 `completed_state_id`。主 agent（主代理）不提交目标状态、转换 ID 或产物列表。Runtime（运行时）再根据当前状态、候选转换、转换条件、Guard Point（守卫点）和画像声明的产物位置决定是否推进状态以及推进到哪里。
