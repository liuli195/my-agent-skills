...
- 显式激活或主 agent（主代理）事件缺少 `required_fields` 或 `identity_fields` 时返回 `no_subject_match`，写审计，不创建实例。
- Hook（钩子）事件无法匹配唯一已有实例时必须忽略：不拒绝、不提示、不写审计。
...
- 显式激活或主 agent（主代理）事件无法解析或解析歧义时写审计，并给出修复建议。Hook（钩子）事件不承担实例存在性检查。
