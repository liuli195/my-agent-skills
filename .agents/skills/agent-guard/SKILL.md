...
3. 生成或更新画像时：读取 `references/extraction-method.md`，先调研被守卫对象，再生成 Guard Profile（守卫画像）草案。
4. 修改画像后：运行 `validate_guard_profile.py <guard-profile-dir>` 校验文件、字段和引用。
5. 写入项目、用户级目录、安装 Hook（钩子）或配置会拒绝操作的 `deny` 状态权限前：先取得用户明确授权。
6. 交付结果：说明产物、校验结果和仍需确认的术语或权限策略。
