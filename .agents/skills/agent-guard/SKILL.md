...
- 生成、更新或初始化 Guard Profile（守卫画像）时，如果本轮没有已确认的 `confirmed-notes.yaml`，必须先读取和使用 `$grill-with-docs`（带文档拷问方法）和用户进行多轮追问来进行调研；禁止直接生成画像、运行提取器或初始化。
...
3. 生成、更新或初始化画像时：先确认本轮是否已有 `grill_with_docs.status: confirmed` 的 `confirmed-notes.yaml`；没有就先必须使用 `$grill-with-docs`（带文档拷问方法）和用户进行多轮拷问来进行调研。
